	...
	testCases := []struct {
	...
	}{
		{
			...
			buildStub: func(store *mockdb.MockStore) {
				...
				store.EXPECT().UpdateAccount(gomock.Any(), gomock.Eq(arg)).Times(1).Return(db.Account{
					ID:        account.ID,
					Balance:   balance,
			...
			checkResponse: func(recorder *httptest.ResponseRecorder) {
				require.Equal(t, http.StatusOK, recorder.Code)
				account.Balance = balance // test against a new balance
				requireBodyMatchAccount(t, recorder.Body, account)
			},
		},
		{
		...
		},
		{
			...
			buildStub: func(store *mockdb.MockStore) {
				...
				store.EXPECT().UpdateAccount(gomock.Any(), gomock.Eq(arg)).Times(0)
			},
			checkResponse: func(recorder *httptest.ResponseRecorder) {
				require.Equal(t, http.StatusBadRequest, recorder.Code)
			},
...
func TestDeleteAccountAPI(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			checkResponse: func(recorder *httptest.ResponseRecorder) {
				require.Equal(t, http.StatusOK, recorder.Code)
				requireBodyMatchJSON(t, recorder.Body, map[string]string{"message": "successfully delete user"})
			},
		},
		{
			name:      "InternalError",
	...
	testCases := []struct {
		...
		},
		{
			...
			buildStub: func(store *mockdb.MockStore) {
				...
				store.EXPECT().ListAccounts(gomock.Any(), gomock.Eq(arg)).Times(0)
			},
			checkResponse: func(recorder *httptest.ResponseRecorder) {
				require.Equal(t, http.StatusBadRequest, recorder.Code)
			},
...
func requireBodyMatchAccounts(t *testing.T, body *bytes.Buffer, accounts []db.Account) {
	...
	for i := range accounts {
		require.Equal(t, gotAccounts[i], accounts[i])
	}
}

func requireBodyMatchJSON(t *testing.T, body *bytes.Buffer, ret map[string]string) {
	data, err := ioutil.ReadAll(body)
	require.NoError(t, err)

	var gotRet map[string]string
	err = json.Unmarshal(data, &gotRet)
	require.NoError(t, err)
	require.Equal(t, gotRet, ret)
}
